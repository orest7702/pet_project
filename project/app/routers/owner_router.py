...
def read_owners(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    ...
    """
    owners = owner_crud.get_owners(db=db, skip=skip, limit=limit)
    if not owners:
        raise HTTPException(
...
def read_owner(owner_id: int, db: Session = Depends(get_db) ):
    ...
    """
    owner = owner_crud.get_owner(db=db, owner_id=owner_id)
    if not owner:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, 
...
def create_owner(owner: OwnerCreate, db: Session = Depends(get_db)):
    ...
    """
    return owner_crud.create_owner(db=db, owner=owner)
...
def delete_owner_endpoint(owner_id: int, db: Session = Depends(get_db)):
    ...
    success = owner_crud.delete_owner(db=db, owner_id=owner_id)
